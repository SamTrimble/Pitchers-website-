...
# 5.23 Mental Performance & Wellness Module

## Purpose
Every athlete profile must include a dedicated **Mental Performance & Wellness** tab. The module allows an athlete to request and receive a personalized mental-performance or wellness plan from an authorized provider. It may support confidence, focus, motivation, pressure management, visualization, routines, communication, sleep habits, recruiting pressure, transition to college, and general well-being.

The platform must clearly distinguish mental-performance coaching, wellness coaching, counseling, and licensed clinical care. It must not present routine app support as emergency care, diagnosis, psychotherapy, or medical treatment.

## Athlete Capabilities
Athletes may:
- request a personalized plan
- select support areas and goals
- request written guidance, a private video response, or a call
- view assigned exercises and resources
- complete reflections and check-ins
- securely message the assigned provider
- view, reschedule, or cancel sessions according to organization policy
- track plan completion and approved progress measures

## Plan Request Fields
- athlete ID and organization ID
- requested provider type
- support categories
- athlete-stated goals and challenges
- preferred communication format
- preferred appointment times
- urgency selection
- consent acknowledgment
- guardian consent status when required
- submitted date and request status

Request statuses: `draft`, `submitted`, `under_review`, `provider_assigned`, `additional_information_requested`, `plan_in_development`, `plan_ready`, `active`, `paused`, `completed`, `declined`, `referred_out`, and `archived`.

## Provider Roles
Authorized providers may include a mental-performance coach, sport-psychology consultant, counselor, licensed mental-health professional, wellness coach, or organization-approved external provider. Provider role, credentials, scope of service, approval status, and organization membership must be stored.

## Personalized Plan Structure
Each plan may contain:
- plan title, objective, dates, and assigned provider
- focus areas and measurable goals
- weekly themes
- breathing, visualization, confidence, journaling, and routine exercises
- pre-game and post-game routines
- educational videos, guided audio, reading, and documents
- check-in questions and habit tracking
- scheduled calls and review dates
- athlete-visible notes and separate provider-only notes

Plan item types include written exercises, videos, audio, reflection prompts, surveys, goals, checklists, scheduled calls, habit trackers, documents, resources, and follow-up tasks.

## Private Video Delivery
The assigned provider must be able to upload or record a private video, assign it to one athlete, add instructions and a due date, require acknowledgment, and review completion. Videos must use secure object storage and signed URLs and must never be public by default.

## Call Scheduling
The module must support introductory calls, one-on-one coaching calls, plan reviews, progress check-ins, parent/guardian meetings, referral discussions, and follow-up sessions. Store provider, athlete, optional guardians, start/end time, timezone, meeting link or location, reminders, cancellation policy, notes, and status.

Session statuses: `requested`, `pending_confirmation`, `confirmed`, `rescheduled`, `completed`, `cancelled`, and `missed`.

## Secure Messaging
Athlete-provider messaging must support text, approved attachments, audit history, reporting, retention policies, and organization-defined guardian visibility for minors. Unrelated coaches must not automatically see private wellness messages or notes.

## Progress Tracking
Track plan completion, exercises, call attendance, routine adherence, reflection completion, goals, and optional self-reported confidence, focus, stress, or readiness. These self-reported measures are informational and must not be interpreted by the platform as a diagnosis.

## Privacy and Permissions
Wellness information must have stricter permissions than standard workout data. Access may be granted to the athlete, assigned provider, permitted guardian, approved supervisor, and limited organization administrators. Standard coaches must not receive access by default. Support separate athlete-visible and provider-only notes, consent records, access logs, retention rules, exports, and immediate permission revocation.

## Safety Workflow
The module must state that the app is not an emergency service. It must support organization-configurable crisis resources, emergency contacts, provider escalation instructions, referral-out workflows, and documented safety actions. Any automated high-risk phrase flag must be treated only as a support signal and require human review.

## Database Tables
Add:
- `wellness_provider_profiles`
- `wellness_provider_credentials`
- `athlete_wellness_requests`
- `athlete_wellness_request_categories`
- `athlete_wellness_plans`
- `athlete_wellness_plan_goals`
- `athlete_wellness_plan_items`
- `athlete_wellness_plan_completions`
- `athlete_wellness_checkins`
- `wellness_sessions`
- `wellness_resources`
- `wellness_video_assignments`
- `wellness_consents`
- `wellness_access_permissions`
- `wellness_referrals`
- `wellness_safety_events`

Every table must include organization scoping, timestamps, audit fields, and appropriate access controls.

## API Endpoints
- `POST /api/v1/athletes/{athlete_id}/wellness/requests`
- `GET /api/v1/athletes/{athlete_id}/wellness/requests`
- `POST /api/v1/wellness/requests/{request_id}/assign-provider`
- `POST /api/v1/wellness/plans`
- `GET /api/v1/athletes/{athlete_id}/wellness/plan`
- `PATCH /api/v1/wellness/plans/{plan_id}`
- `POST /api/v1/wellness/plans/{plan_id}/items`
- `POST /api/v1/wellness/items/{item_id}/complete`
- `POST /api/v1/wellness/videos/upload-url`
- `POST /api/v1/wellness/sessions`
- `PATCH /api/v1/wellness/sessions/{session_id}`
- `POST /api/v1/wellness/checkins`
- `POST /api/v1/wellness/consents`
- `POST /api/v1/wellness/referrals`
- `POST /api/v1/wellness/safety-events`

Every endpoint must enforce organization isolation, provider assignment, consent, and role permissions.

## Billing Options
Organizations may sell a monthly mental-performance subscription, a one-time personalized plan, individual video feedback, a single coaching call, multi-session packages, premium development plans, or team packages. Billing access must never override privacy permissions.

## Notifications
Notify users when a request is submitted, a provider is assigned, more information is needed, a plan is ready, a video is available, a call is requested or confirmed, a session is approaching, a secure message arrives, a check-in is due, or a plan review is due. Notification previews must avoid exposing sensitive details.

## Acceptance Criteria
1. Every athlete profile includes a Mental Performance & Wellness tab.
2. An athlete can request a personalized plan.
3. An authorized provider can accept and manage the request.
4. The provider can publish a private plan and private video.
5. The athlete can request or schedule a call.
6. Athlete and provider can securely message.
7. Plan items and progress can be tracked.
8. Provider-only notes remain private.
9. Guardian access follows consent rules.
10. Unrelated coaches are denied access by default.
11. Safety notices and escalation workflows are available.
12. All access and changes are auditable.
13. Automated tests verify permission boundaries.
